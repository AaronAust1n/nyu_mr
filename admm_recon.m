...
%         x = pcg(f,b,1e-6,cg_iter,[],[],x);
    ...
    [c, idx] = max(c, [], 2);
%     for q=size(x,1):-1:1
%         [c(q),idx(q)] = max(x(q,:) * conj(m_dict), [], 2);
%     end
    ...
    if verbose == 1
        ...
        if length(recon_dim) == 4
            tmp = abs(repmat(c(:,:,slices), [1 1 1 length(t)]) .* D(:,:,slices,t));
            tmp = array2mosaic(tmp(:,:,:), [length(t) length(slices)]);
        else
            tmp = abs(repmat(c, [1 1 length(t)]) .* D(:,:,t));
            tmp = array2mosaic(tmp);
        end
        sfig(234); imagesc(tmp); title(['P(x) - iteration = ', num2str(j)]); colorbar
        
        if length(recon_dim) == 4
            tmp = abs(x(:,:,slices,t) - repmat(c(:,:,slices), [1 1 1 length(t)]) .* D(:,:,slices,t));
            tmp = array2mosaic(tmp(:,:,:), [length(t) length(slices)]);
        else
            tmp = abs(x(:,:,t) - repmat(c, [1 1 length(t)]) .* D(:,:,t));
            tmp = array2mosaic(tmp);
        end
